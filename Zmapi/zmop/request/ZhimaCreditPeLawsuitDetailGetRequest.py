...
:author: auto create
:date: 2017-01-18 17:45:07
'''


class ZhimaCreditPeLawsuitDetailGetRequest:
    def __init__(self):
        self.__lawsuitId = '' # 涉诉类型明细ID，对应字段值：裁判文书（partyId)，执行公告(zxggId)，失信记录(shixinId)，网贷黑名单(wdhmdId)，曝光台(bgtId)
        self.__lawsuitType = '' # 涉诉类型包括：裁判文书（party)，执行公告(zxgg)，失信记录(sxgg)，网贷黑名单(wdhmd)，曝光台(bgt)
